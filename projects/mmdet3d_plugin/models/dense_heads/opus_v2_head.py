...
from spconv.pytorch import (SparseConvTensor, SparseConv3d, SubMConv3d,
                            SparseSequential)
from ..utils.opus_utils import decode_points, encode_points
...
class Densifier(BaseModule):
    ...
    def _forward_impl(self, pts_feats, refine_pts):
        ...
        coors = coors[..., 0] * self.scale_xyz + coors[..., 1] * self.scale_yz + coors[..., 2] * self.scale_z + coors[..., 3]
    ...
    def forward(self, pts_feats, refine_pts):
        ...
        if self.with_cp and self.training and (
                pts_feats.requires_grad or refine_pts.requires_grad):
            return cp(self._forward_impl, pts_feats, refine_pts,
                      use_reentrant=False)
        return self._forward_impl(pts_feats, refine_pts)
...
class OPUSV2Head(BaseModule):
    ...
    def _get_layer_loss_weights(self, key, num_layers):
        weights = self.train_cfg.get(key, None)
        if weights is None:
            return [1.0] * num_layers
        if len(weights) != num_layers:
            raise ValueError(
                f'{key} length mismatch: expected {num_layers}, got {len(weights)}')
        return weights
    ...
    def loss_single(self,
        ...
        if (cls_scores is not None) and (voxel_coors is not None):
            ...
            avg_factor = int((gt_labels != self.empty_label).sum().item())
            avg_factor = max(avg_factor, self.train_cfg.get('cls_avg_factor_min', 1))
    ...
    def loss(self, voxel_semantics, mask_camera, preds_dicts):
        ...
        cls_layer_weights = self._get_layer_loss_weights(
            'cls_layer_loss_weights', num_dec_layers)
        pts_layer_weights = self._get_layer_loss_weights(
            'pts_layer_loss_weights', num_dec_layers)
        ...
        loss_dict['loss_cls'] = losses_cls[-1] * cls_layer_weights[-1]
        loss_dict['loss_pts'] = losses_pts[-1] * pts_layer_weights[-1]

        # loss from other decoder layers
        num_dec_layer = 0
        for loss_cls_i, loss_pts_i, cls_weight_i, pts_weight_i in zip(
                losses_cls[:-1], losses_pts[:-1],
                cls_layer_weights[:-1], pts_layer_weights[:-1]):
            loss_dict[f'd{num_dec_layer}.loss_cls'] = loss_cls_i * cls_weight_i
            loss_dict[f'd{num_dec_layer}.loss_pts'] = loss_pts_i * pts_weight_i
            num_dec_layer += 1
        return loss_dict
