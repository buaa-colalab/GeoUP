...
class StreamPETRHead(AnchorFreeHead):
    ...
    def pre_update_memory(self, data):
        x = data['prev_exists']
        prev_exists = x.to(dtype=self.reference_points.weight.dtype)
        B = x.size(0)
        # refresh the memory when the scene changes
        if self.memory_embedding is None:
            self.memory_embedding = prev_exists.new_zeros(B, self.memory_len, self.embed_dims)
            self.memory_reference_point = prev_exists.new_zeros(B, self.memory_len, 3)
            self.memory_timestamp = prev_exists.new_zeros(B, self.memory_len, 1)
            self.memory_egopose = prev_exists.new_zeros(B, self.memory_len, 4, 4)
            self.memory_velo = prev_exists.new_zeros(B, self.memory_len, 2)
        else:
            ...
            self.memory_egopose = memory_refresh(self.memory_egopose[:, :self.memory_len], prev_exists)
            self.memory_velo = memory_refresh(self.memory_velo[:, :self.memory_len], prev_exists)
        ...
        if self.num_propagated > 0:
            ...
            self.memory_reference_point[:, :self.num_propagated]  = self.memory_reference_point[:, :self.num_propagated] + (1 - prev_exists).view(B, 1, 1) * pseudo_reference_points
            self.memory_egopose[:, :self.num_propagated]  = self.memory_egopose[:, :self.num_propagated] + (1 - prev_exists).view(B, 1, 1, 1) * torch.eye(4, device=x.device, dtype=self.memory_egopose.dtype)
