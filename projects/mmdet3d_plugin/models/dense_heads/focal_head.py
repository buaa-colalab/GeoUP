...
class FocalHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if self.multi_dataset_num_classes is not None:
            if self.use_sigmoid_cls:
                self.multi_dataset_cls_out_channels = [num_classes for num_classes in self.multi_dataset_num_classes] 
            else:
                self.multi_dataset_cls_out_channels = [num_classes + 1 for num_classes in self.multi_dataset_num_classes] 
    ...
    def _init_layers(self):
        if self.multi_dataset_num_classes is not None:
            self.cls_list = nn.ModuleList([
                nn.Conv2d(self.embed_dims, num_classes, kernel_size=1) 
                for num_classes in self.multi_dataset_num_classes
            ])
        else:
    ...
    def normalize_intrinsics(self, K, w, h):
        ...
        K: (B,3,3) 原始内参矩阵
        w,h: 当前图像宽高
        return: (B,4) [fx', fy', cx', cy'] 全部归一化到 0~1
        """
        fx = K[:, 0, 0] / max(w, h)   # 用 max(w,h) 保证 fx/fy 同尺度
        fy = K[:, 1, 1] / max(w, h)
        cx = K[:, 0, 2] / w
        cy = K[:, 1, 2] / h
        return torch.stack([fx, fy, cx, cy], dim=-1)   # (B,4)

    @force_fp32(apply_to=('location', 'img_feats'))
    def forward(self, location, **data):
        ...
        x = src.flatten(0, 1)
        if self.use_intrinsics:
            _, _, _, H, W = data['img'].shape
            x = x.reshape(bs * n, c, h * w).permute(0,2,1)
            norm_intrinsics = self.normalize_intrinsics(data['intrinsics'], W, H)
            norm_intrinsics = norm_intrinsics.unsqueeze(1).repeat(1, x.shape[1], 1)
            x = self.cam_embeding(x, norm_intrinsics)
            x = x.reshape(bs * n, h, w, c).permute(0,3,1,2)
        cls_feat = self.shared_cls(x)
        if self.multi_dataset_num_classes is not None:
            cls_idx = self._get_cls_head_idx(data['dataset'][0])
            cls = self.cls_list[cls_idx](cls_feat)
            num_classes = self.multi_dataset_num_classes[cls_idx]
            for i in range(len(self.cls_list)):
                    if i != cls_idx:
                        temp = self.cls_list[i](cls_feat)[:, :cls.shape[1]] * 0.0
                        cls[:, :temp.shape[1]] = cls[:, :temp.shape[1]] + temp
        else:
            cls = self.cls(cls_feat)
            num_classes = self.num_classes
        centerness = self.centerness(cls_feat)
        cls_logits = cls.permute(0,2,3,1).reshape(bs*n,-1,num_classes)
    ...
    def loss(self,
        ...
        all_dataset_idx_list = [dataset_idx for _ in range(len(all_gt_bboxes2d_list))]
        if len(all_gt_bboxes2d_list) == 0:
            zero = enc_cls_scores.sum() * 0
            loss_dict['enc_loss_cls'] = zero
            loss_dict['enc_loss_bbox'] = zero
            loss_dict['enc_loss_iou'] = zero
            loss_dict['centers2d_losses'] = zero
            loss_dict['centerness_losses'] = zero
            return loss_dict
        enc_loss_cls, enc_losses_bbox, enc_losses_iou, centers2d_losses, centerness_losses = \
            self.loss_single(enc_cls_scores, enc_bbox_preds, pred_centers2d, centerness,
                                ...
                                all_depths_list, img_metas, gt_bboxes_ignore, all_dataset_idx_list)
    ...
    def loss_single(self,
        ...
        cls_reg_targets = self.get_targets(cls_scores_list, bbox_preds_list, centers2d_preds_list,
                                             ...
                                             all_depths_list, img_metas, gt_bboxes_ignore_list, all_dataset_idx_list)
        ...
        # classification loss
        dataset_idx = all_dataset_idx_list[0]
        if self.multi_dataset_num_classes:
            cls_out_channels = self._get_cls_out_channels_for_dataset(dataset_idx)
        else:
    ...
    def get_targets(self,
        ...
        (labels_list, label_weights_list, bbox_targets_list, bbox_weights_list,
         centers2d_targets_list, pos_inds_list, neg_inds_list) = multi_apply(
            ...
            gt_bboxes_list, gt_labels_list, all_centers2d_list,
            all_depths_list, img_meta_list, gt_bboxes_ignore_list, all_dataset_idx_list)
