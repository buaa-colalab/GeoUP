...
class cuRoPE2D(torch.nn.Module):
    ...
    def forward(self, tokens, positions):
        # The CUDA kernel expects [B, N, H, D] contiguous tokens and [B, N, 2]
        # contiguous positions, while the attention code uses [B, H, N, D].
        tokens_nhd = tokens.transpose(1, 2).contiguous()
        positions = positions.contiguous()
        tokens_nhd = cuRoPE2D_func.apply(tokens_nhd, positions, self.base, self.F0)
        return tokens_nhd.transpose(1, 2)
