        ...
        // const int N, const int H, const int D )
{
    ...
    */
    ...
    for (int h = 0; h < H; h++)
    {
        ...
        shared[threadIdx.x] = scalar_to_float(tokens[b][n][h][threadIdx.x]);
        __syncthreads();
        ...
        // write output
        if ((threadIdx.x % (D/2)) < Q)
            tokens[b][n][h][threadIdx.x] = float_to_scalar<scalar_t>(u*cos - v*sin);
        else
            tokens[b][n][h][threadIdx.x] = float_to_scalar<scalar_t>(v*cos + u*sin);
    }
}
