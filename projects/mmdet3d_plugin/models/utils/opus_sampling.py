...
from .opus_utils import decode_points, rotation_3d_in_axis
...
def sampling_4d(sample_points, mlvl_feats, scale_weights, occ2img,
                image_h, image_w, num_views=6, eps=1e-5,
    ...
    Args:
        ...
        image_h/image_w: padded image size used to normalize coordinates for
            feature sampling.
        valid_h/valid_w: unpadded image size used to reject points that only
            fall inside padded pixels. Defaults to image_h/image_w.
    Symbol meaning:
        B: batch size
        Q: num of queries
        T: num of frames
    ...
    N = num_views
    valid_h = image_h if valid_h is None else valid_h
    valid_w = image_w if valid_w is None else valid_w
    ...
    sample_points_pixel = sample_points_cam.clone()
    ...
    valid_mask = ((homo > eps) 
        & (sample_points_pixel[..., 1:2] > 0.0)
        & (sample_points_pixel[..., 1:2] < valid_h)
        & (sample_points_pixel[..., 0:1] > 0.0)
        & (sample_points_pixel[..., 0:1] < valid_w)
