...
def invert_rigid_transform_torch(matrix: torch.Tensor) -> torch.Tensor:
    """
    Computes the inverse of a 4x4 rigid body transformation matrix in PyTorch.
    A rigid body transformation matrix has the form:
    [ R | t ]
    [ 0 | 1 ]
    ...
    [ 0   |    1     ]
    Args:
        matrix (torch.Tensor): A tensor of shape (4, 4) or (B, 4, 4)
                               representing one or a batch of transformation matrices.
    Returns:
        torch.Tensor: The inverted matrix/matrices of the same shape as input.
    """
    inverse_matrix = torch.zeros_like(matrix)
    inverse_matrix[..., 3, 3] = 1.0
    rotation = matrix[..., :3, :3]
    translation = matrix[..., :3, 3]
    inv_rotation = rotation.transpose(-2, -1)
    inv_translation = -torch.matmul(inv_rotation, translation.unsqueeze(-1)).squeeze(-1)
    inverse_matrix[..., :3, :3] = inv_rotation
    inverse_matrix[..., :3, 3] = inv_translation
