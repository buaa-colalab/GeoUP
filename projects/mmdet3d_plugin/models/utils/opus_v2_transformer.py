...
from .opus_utils import decode_points, encode_points, inverse_sigmoid
from torch.utils.checkpoint import checkpoint as cp

# Check if CUDA extension is available
try:
    from .csrc.wrapper import MSMVSamplingC2345
    MSMV_CUDA = True
except ImportError:
    MSMV_CUDA = False
...
class OPUSV2Transformer(BaseModule):
    def __init__(self,
        ...
        self.decoder = OPUSTransformerDecoder(
            ...
            num_refines, num_groups, num_pt_channels, scales, pc_range=pc_range)
    ...
    def forward(self, query_points, query_feat, mlvl_feats, img_metas):
        ...
        cls_scores = [None if s is None else torch.nan_to_num(s) for s in cls_scores]
...
class OPUSTransformerDecoder(BaseModule):
    ...
    def forward(self, query_points, query_feat, mlvl_feats, img_metas):
        ...
        # organize projections matrix and move them to the feature device
        ego2img = torch.stack([m['ego2img'] for m in img_metas], dim=0).to(
            device=query_feat.device, dtype=query_feat.dtype, non_blocking=True)  # [B, TN, 4, 4]
        ego2occ = torch.stack([m['ego2occ'] for m in img_metas], dim=0).to(
            device=query_feat.device, dtype=query_feat.dtype, non_blocking=True)  # [B, 4, 4]
        occ2ego = torch.inverse(ego2occ)
        occ2img = ego2img @ occ2ego[:, None]
...
class OPUSTransformerDecoderLayer(BaseModule):
    ...
    def forward(self, query_points, query_feat, mlvl_feats, occ2img, img_metas):
        ...
        if self.training or self.last_layer:
            ...
            cls_score = cls_score.reshape(B, Q, self.num_refines, -1)
...
class OPUSSampling(BaseModule):
    ...
    def inner_forward(self, query_points, query_feat, mlvl_feats, occ2img, img_metas):
        ...
        '''
        B, Q = query_points.shape[:2]
        valid_h, valid_w, _ = img_metas[0]['img_shape'][0]
        if 'pad_shape' in img_metas[0]:
            image_h, image_w, _ = img_metas[0]['pad_shape'][0]
        else:
            image_h, image_w = valid_h, valid_w
